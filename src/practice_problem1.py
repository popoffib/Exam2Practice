...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        if len(self.initial_contents) > self.initial_volume:
            self.contents = ''
        self.count = []
        self.end_count = 0
    ...
    def double(self):
        ...
        #    ** TWO **   LINES OF CODE.
        ################################################################
        new = self.append_string(self.contents)
        return new
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) > self.volume:
            for k in range(0, self.volume):
                start = start + self.initial_contents[k]
        else:
            start = self.contents
        self.contents = start

        for k in range(new_volume, len(self.initial_contents)):
            end = end + self.initial_contents[k]
    ...
    def double_then_shrink(self, new_volume):
        ...
        d_then_s = len(double) + len(shrink)
        return d_then_s
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        contents = self.initial_contents
        volume = self.initial_contents
